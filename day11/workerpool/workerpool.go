...
// Worker pool driver
func Run[J any, R any](ctx context.Context, jobs []J, processFunc func(job J, startTime time.Time) R, rl *ratelimiter.RateLimiter) <-chan R {
	...
	for i := 1; i <= WorkerPoolSize; i++ {
		wg.Go(func() {
			worker(ctx, jobsCh, resultCh, processFunc, rl)
		})
	}
...
// Worker
func worker[J any, R any](ctx context.Context, jobsCh <-chan J, resultCh chan<- R, processFunc func(job J, startTime time.Time) R, rl *ratelimiter.RateLimiter) {
	startTime := time.Now()
	for {
		select {
		case job, ok := <-jobsCh:
			...
			// try to see if a new token can be acquired if one is available
			rl.Acquire()
